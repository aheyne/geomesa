...
object GeoMesaBatchWriterConfig extends LazyLogging {
  ...
  protected[util] def buildBWC: BatchWriterConfig = {
    ...
    val latency = fetchProperty(GeoMesaProperties.GEOMESA_BATCHWRITER_LATENCY_MILLS)
        .getOrElse(GeoMesaProperties.GEOMESA_BATCHWRITER_LATENCY_MILLS.default.toLong)
    ...
    val memory = fetchMemoryProperty(GeoMesaProperties.GEOMESA_BATCHWRITER_MEMORY)
        .getOrElse(GeoMesaProperties.GEOMESA_BATCHWRITER_MEMORY.default.toLong)
    logger.trace(s"GeoMesaBatchWriter config: maxMemory set to $memory bytes.")
    bwc.setMaxMemory(memory)

    val threads = fetchProperty(GeoMesaProperties.GEOMESA_BATCHWRITER_MAXTHREADS).map(_.toInt)
        .getOrElse(GeoMesaProperties.GEOMESA_BATCHWRITER_MAXTHREADS.default.toInt)
    logger.trace(s"GeoMesaBatchWriter config: maxWriteThreads set to $threads.")
    bwc.setMaxWriteThreads(threads.toInt)

    fetchProperty(GeoMesaProperties.GEOMESA_BATCHWRITER_TIMEOUT_MILLS).foreach { timeout =>
